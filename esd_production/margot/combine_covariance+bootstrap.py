...
if 'bootstrap' in purpose:
	print 'Step 4: Combine the bootstrap samples into the ESD profiles and covariance matrix'
else:
	print 'Step 2: Combine the splits into the ESD profiles and covariance matrix'
print
...
#		filename_N1 = filename_var.replace('binnum', '%i'%(N1+1))
		filename_N1 = shear.define_filename_var(purpose, centering, rankmin, rankmax, Nfofmin, Nfofmax, ZBmin, ZBmax, binname, N1+1, Nobsbins, obslim, obslim_min, obslim_max, path_Rbins, O_matter, h) # Defining the name of the output files
		if ('random' or 'star') in purpose:
		...
		shearcatname_N1 = shear.define_filename_splits(path_splits, purpose, filename_N1, 0, 0, filename_addition, blindcat) # Paths to the resulting files
...
#				filename_N2 = filename_var.replace('binnum', '%i'%(N2+1))
				filename_N2 = shear.define_filename_var(purpose, centering, rankmin, rankmax, Nfofmin, Nfofmax, ZBmin, ZBmax, binname, N2+1, Nobsbins, obslim, obslim_min, obslim_max, path_Rbins, O_matter, h) # Defining the name of the output files
				if ('random' or 'star') in purpose:
					filename_N2 = '%i_%s'%(Ncat, filename_N2) # Ncat is the number of existing randoms
				
				shearcatname_N2 = shear.define_filename_splits(path_splits, purpose, filename_N2, 0, 0, filename_addition, blindcat) # Paths to the resulting files
				...
				for R1 in xrange(nRbins):
					for R2 in range(nRbins):
						...
						ESD_mask = (0 < error_N1[:, R1])&(error_N1[:, R1] < inf) & (0 < error_N2[:, R2])&(error_N2[:, R2] < inf) # Mask values that do not have finite values (inf or nan)

						if sum(ESD_mask) > 0: # If there are any values that are not masked
							
							cov[N1,N2,R1,R2] = np.cov((ESDt_N1[:, R1])[ESD_mask], (ESDt_N2[:, R2])[ESD_mask], bias=1)[0,1]

	#						print N1+1, N2+1, R1+1, R2+1, cov[N1,N2,R1,R2]
...
#		filename_N1 = filename_var.replace('binnum', '%i'%(N1+1))
		...
		for x in xrange(len(kidscats)):
			...
			if os.path.isfile(shearcatname_N1):
				...
				gammat[N1] = gammat[N1] + sum(lfweights*(Cs_N1*e1+Ss_N1*e2),0) # The tangential shear
				gammax[N1] = gammax[N1] + sum(lfweights*(-Ss_N1*e1+Cs_N1*e2),0) # The cross shear
				wk2[N1] = wk2[N1] + sum(lfweights*Zs_N1,0) # The total weight (lensfit weight + lensing efficiency)
...
#					filename_N2 = filename_var.replace('binnum', '%i'%(N2+1))
					...
					shearcatname_N2 = shear.define_filename_splits(path_splits, purpose, filename_N2, kidscats[x], 0, filename_addition, blindcat)
