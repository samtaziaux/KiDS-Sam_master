...
# Input for the codes
inputlist = (center, rankmin, rankmax, Nfofmin, Nfofmax, ZBmin, ZBmax, name_obsbins, path_obsbins, Nobsbins, name_obslim, obslim_min, obslim_max, path_Rbins, path_output, purpose, O_matter, h0, filename_addition, path_kidscats)
inputstring = '%s %g %g %g %g %g %g %s %s %i %s %s %s %s %s %s %g %g %s %s'%inputlist
...
if purpose == 'binlimits':
	...
	selname = 'python define_obsbins_equal_sn.py %i %i %i %s %s &'%(Ncore, Ncores, binnum, blindcat, inputstring)
	...
	for n in xrange(Nruns):
		...
		for binnum in np.arange(Nobsbins)+1:
			for Ncore in np.arange(Ncores)+1:
				...
				splitsname = 'python -W ignore shear+covariance.py %i %i %i %s %s &'%(Ncore, Ncores, binnum, blindcat, inputstring)
	...
	if ('bootstrap' in purpose) or ('catalog' in purpose):
		...
		ps = []
		combname = 'python -W ignore combine_splits.py %i %i %i %s %s &'%(Ncore, Ncores, binnum, blindcat, inputstring)
	...
	if ('bootstrap' in purpose) or ('catalog' in purpose):
		...
		for blindcat in blindcats:
			plotname = 'python -W ignore stack_shear+bootstrap.py %i %i %i %s %s &'%(Ncore, Ncores, binnum, blindcat, inputstring)
	...
	if ('bootstrap' in purpose) or ('covariance' in purpose):
		...
		ps = []
		for blindcat in blindcats:
			combname = 'python -W ignore combine_covariance+bootstrap.py %i %i %i %s %s &'%(Ncore, Ncores, binnum, blindcat, inputstring)
			runcomb = sub.Popen(shlex.split(combname))
			ps.append(runcomb)
		for p in ps:
			p.wait()
	...
	if ('bootstrap' in purpose) or ('covariance' in purpose):
		ps = []
		for blindcat in blindcats:
			combname = 'python -W ignore plot_covariance+bootstrap.py %i %i %i %s %s &'%(Ncore, Ncores, binnum, blindcat, inputstring)
			runcomb = sub.Popen(shlex.split(combname))
			ps.append(runcomb)
		for p in ps:
			p.wait()
