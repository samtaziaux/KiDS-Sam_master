...
def model(theta, R, calculate_covariance=False):
    ...
    for i, observable in enumerate(observables):
        if observable.obstype == 'gm':
            ...
            #idx_gm = idx_slice(nbins, nbins+nbins_gm)
            #print('idx_gm =', idx_gm)
            nbins += nbins_gm
    ...
    ###
    # k-space filtered profile for CMB lensing
    ###
    """
    print('rvir_range_lin =', rvir_range_lin.shape)
    print('mass_range =', mass_range.shape)
    print('concentration =', concentration.shape)
    print('z =', z.shape)
    print('rho_bg =', rho_bg.shape)
    # try this out later
    #prof = profiles.NFW(mass_range, concentration, z)
    #surface_density = prof.surface_density(rvir_range_lin)
    #print('surface_density =', surface_density.shape)
    delta_c = nfw.delta(concentration, setup['delta'])
    sigma_s = (rvir_range_lin/concentration) * delta_c * rho_bg
    # calcular el perfil hasta 3*r200
    x = np.linspace(0, 3, 800)[:,None] * rvir_range_lin
    print('x =', x.shape)
    print('sigma_s =', sigma_s.shape)
    # this has shape (z.size,x.size,mass_range.size)
    surface_density = nfw.sigma(x, sigma_s[:,None])
    print('surface_density =', surface_density.shape)
    d = cosmo_model.angular_diameter_distance(z)
    print('d =', d.shape)
    if setup['distances'] == 'comoving':
        d = d * (1+z)
        print('d =', d.shape)
    surface_density_theta = surface_density * d[:,None,None] * pixell.utils.arcmin
    kappa = surface_density_theta / sigma_crit(cosmo_model, z, zs)
    """

    """
    # filtrar en espacio de Fourier
    ...
    d = cosmo_model.angular_diameter_distance(z).to(u.Mpc).value
    print('d =', d.shape)
    theta = d
    # I think we need a uniformly-sampled function
    #fb = wise_utils.FilterBinner(
    ###
    # end
    ###
    """
    ...
    if ingredient_gm:
        ...
        if setup['return'] == 'power':
            # this needs to include the volume element per z
            if integrate_zlens:
    ...
    if ingredient_gm:
        if integrate_zlens:
            # I need to include V(z) in the redshift integration!
    ...
    if ingredient_gm:
        ...
        #print('1 surf_dens2 =', surf_dens2, surf_dens2.shape)
        ...
        #print('2 surf_dens2 =', surf_dens2, surf_dens2.shape)
        ...
        #print('3 surf_dens2 =', surf_dens2, surf_dens2.shape)
        ...
        if setup['return'] in ('kappa', 'sigma'):
            ...
            kmask = '/home/cristobal/Documents/actpol/madcows/wisepan/wisepan_kmask.fits'
            # this applies to the kappa profile
            th_binned = []
