...
def main():
	...
	# Input parameters
    #Nsplit, Ncores, centering, rankmin, rankmax, Nfofmin, Nfofmax, \
        #ZBmin, ZBmax, binname, binnum, path_obsbins, Nobsbins, obslim, \
        #obslim_min, obslim_max, path_Rbins, path_output, path_catalogs, \
        #path_splits, path_results, purpose, O_matter, O_lambda, h, \
        #filename_addition, Ncat, splitslist, blindcat, blindcatnum, \
        #path_kidscats = shear.input_variables()
    Nsplit, Ncores, centering, lens_selection, src_selection, \
        binparam, binnum, Nobsbins, \
        path_Rbins, path_output, path_catalogs, \
        path_splits, path_results, purpose, O_matter, O_lambda, h, \
        filename_addition, Ncat, splitslist, blindcat, blindcatnum, \
        path_kidscats = shear.input_variables()
...
#	Edogals = np.loadtxt('lensID_N5.dat')
	...
	print
	print '%s split'%purpose, Nsplit+1, '/', Ncores, ', Center definition = %s'%centering
	print
...
#	kidscats = ['KIDS_130p0_m0p5']#, 'KIDS_130p0_0p5', 'KIDS_131p0_0p5', 'KIDS_130p0_1p5'] # corresponds to Edo's test group: group ID 100065 (galaxy ID 214484)
	...
	splitkidscats = np.array(shear.split(kidscats, Ncores))
...
#							print 'Bin', r+1, 'gammat_A', gammat_tot_A
	...
	if 'catalog' in purpose:
		filename = shear.define_filename_splits(path_splits, purpose, filename_var, Nsplit+1, Ncores, filename_addition, blindcat)
