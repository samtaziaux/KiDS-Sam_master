...
def T_n(n, rho_mean, z, M, R, h_mass, profile, f, omegab, omegac, slope, r_char, sigma, alpha, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2):
    ...
    if profile == 'dm':
        for i in range(len(M)):
            ...
            Ti = (mp.mpf(4.0 * np.pi)/(M[i] * (mp.factorial(2.0*n + 1.0)))) * mp.mpf(Integrate((i_range**(2.0 * (1.0 + n))) * (NFW(rho_mean, Con(z, M[i], f), i_range)), i_range))
...
def multi_proc_T(a,b, que, n, rho_mean, z, m_x, r_x, h_mass, profile, f, omegab, omegac, slope, r_char, sigma, alpha, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2):
	outdict = {}
	...
	for i in range(len(r)):
		T[i,:] = T_n(r[i], rho_mean, z, m_x, r_x, h_mass, profile, f, omegab, omegac, slope, r_char, sigma, alpha, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2)
...
def T_table_multi(n, rho_mean, z, m_x, r_x, h_mass, profile, f, omegab, omegac, slope, r_char, sigma, alpha, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2):
	...
	for j in range(nprocs):
		...
		work = multi.Process(target=multi_proc_T, args=((j*chunk), ((j+1)*chunk), q1, n, rho_mean, z, m_x, r_x, h_mass, profile, f, omegab, omegac, slope, r_char, sigma, alpha, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2))
		procs.append(work)
		work.start()
...
def T_table(n, rho_mean, z, m_x, r_x, h_mass, profile, f, omegab, omegac, slope, r_char, sigma, alpha, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2):
	...
	for i in range(0, n/2, 1):
		T[i,:] = T_n(i, rho_mean, z, m_x, r_x, h_mass, profile, f, omegab, omegac, slope, r_char, sigma, alpha, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2)
        #pbar.update(i+1)
...
def model(theta, R, h=0.7, Om=0.315, Ol=0.685, rmax=2):
    ...
    if taylor_procedure is 1.0:
        T_dm = np.array([T_table(expansion, rho_dm, z, mass_range_lin, radius_range_lin, i[0], "dm", f, omegab, omegac, 0, 0, sigma_c, alpha_s, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2) for i in izip(hod_mass)])
    ...
    if include_baryons is 1.0:
        T_dm = np.array([T_table(expansion, rho_dm, z, mass_range_lin, radius_range_lin, i[0], "dm", f, omegab, omegac, 0, 0, sigma_c, alpha_s, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2) for i in izip(hod_mass)])
        T_stars = np.array([T_table(expansion_stars, rho_mean, z, mass_range_lin, radius_range_lin, i[0], "stars", f, omegab, omegac, alpha_star, r_t0, sigma_c, alpha_s, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2) for i in izip(hod_mass)])
        T_gas = np.array([T_table(expansion, rho_mean, z, mass_range_lin, radius_range_lin, i[0], "gas", f, omegab, omegac, beta_gas, r_c0, sigma_c, alpha_s, A, M_1, gamma_1, gamma_2, b_0, b_1, b_2) for i in izip(hod_mass)])
    ...
    if taylor_procedure is 1.0 or include_baryons is 1.0:
    ...
    if include_baryons is 1.0:
        ...
        Pgas_c = np.array([F_k1 * baryons.GGas_cen_spectrum(hmf, z, F_i, rho_gas_i, rho_mean, f, expansion, pop_c_i, ngal_i, k_range_lin, radius_range_lin, mass_range_lin, T_gas_i, T_tot_i) for F_i, rho_gas_i, pop_c_i, ngal_i, T_gas_i, T_tot_i in izip(F, rho_gas, pop_c, ngal, T_gas, T_tot)])
        Pgas_s = np.array([F_k1 * baryons.GGas_sat_spectrum(hmf, z, F_i, rho_gas_i, rho_mean, f, expansion, pop_s_i, ngal_i, k_range_lin, radius_range_lin, mass_range_lin, T_dm_i, T_gas_i, T_tot_i) for F_i, rho_gas_i, pop_s_i, ngal_i, T_dm_i, T_gas_i, T_tot_i in izip(F, rho_gas, pop_s, ngal, T_dm, T_gas, T_tot)])
    else:
        pass
