...
user-provided lens redshift distribution (see below). *WARNING*: this feature is experimental and, for the time being, its 
use is discouraged. In addition, seting ``nzlens`` to ``True`` significantly slows down the halo model computation.
