...
def loop(purpose, Nsplits, Nsplit, output, outputnames, gamacat, centering,
    ...
    if cat_version == 2:
        for kidscatname in splitkidscats[Nsplit]:
            ...
            print('Analysing part {0}/{1}, process {2}:' \
                  ' {3} (contains {4} objects)'.format(
            ...
            for l in xrange(len(lenssplits)-1):
                ...
                print(lenssplits[l], '-', lenssplits[l+1])
...
def main(nsplit, nsplits, nobsbin, blindcat, config_file, fn):
    ...
    if cat_version == 2:
        ...
        for kidscatname in kidscats:
            i += 1
            print('	{0}/{1}: {2}'.format(i, len(kidscats), kidscatname))
    ...
    if cat_version == 3:
        ...
        print('Importing KiDS catalogs from: {0}'.format(path_kidscats))
        i = 0
        for kidscatname in kidscatname2:
            i += 1
            print('	{0}/{1}: {2}'.format(i, len(kidscatname2), kidscatname))
    ...
    print()
    print('Using {0} cores to create split catalogues'.format(Nsplits),
          end='')
    print(' - Center definition = {0}'.format(centering))
    print()
